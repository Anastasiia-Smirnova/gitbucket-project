...
resource "aws_lb_target_group" "gitbucket_lb_tg_blue" {
  name     = "gitbucket-lb-tg-blue"
  port     = 8080
  protocol = "HTTP"
  vpc_id   = aws_vpc.gitbucket_vpc.id
}

resource "aws_lb_target_group" "gitbucket_lb_tg_green" {
  name     = "gitbucket-lb-tg-green"
  port     = 8080
  protocol = "HTTP"
  vpc_id   = aws_vpc.gitbucket_vpc.id
}
...
resource "aws_lb_listener" "gitbucket_lb_listener" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = var.active_environment == "blue" ? aws_lb_target_group.gitbucket_lb_tg_blue.arn : aws_lb_target_group.gitbucket_lb_tg_green.arn
  }
}
