resource "aws_iam_role" "gitbucket_ec2_iam_role" {
...
resource "aws_iam_role_policy_attachment" "gitbucket_s3_war_access_attachment" {
  role       = aws_iam_role.gitbucket_ec2_iam_role.name
...
resource "aws_iam_instance_profile" "gitbucket_ec2_instance_profile" {
  name = "${var.gitbucket_instance_profile}-${terraform.workspace}"
  role = aws_iam_role.gitbucket_ec2_iam_role.name
}

resource "aws_iam_role_policy" "gitbucket-ec2-secrets-policy" {
  name = "${var.gitbucket_secrets_policy}-${terraform.workspace}"
  role = aws_iam_role.gitbucket_ec2_iam_role.id
